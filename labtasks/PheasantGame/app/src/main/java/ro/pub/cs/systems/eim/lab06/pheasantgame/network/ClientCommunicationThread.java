        ...
        try {
            ...
            if (word.length() > 2) {
                ...
                clientHistoryTextView.post(new Runnable() {
                    @Override
                    public void run() {
                        clientHistoryTextView.setText("Client sent word "
                                + word + " to server\n"
                ...
                handler.post(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(context, "Word must be at least 2 characters long!",
            ...
            Log.d(Constants.TAG, "[CLIENT] Received \"" + response
                    + "\", most recent word was \"" + mostRecentWordSent
                    + "\" on socket " + socket);

            clientHistoryTextView.post(new Runnable() {
                @Override
                public void run() {
                    clientHistoryTextView.setText("Client received word " + response
                            + " from server\n" + clientHistoryTextView.getText().toString());
