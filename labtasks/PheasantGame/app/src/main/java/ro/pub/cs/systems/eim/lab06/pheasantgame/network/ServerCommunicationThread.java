...
public class ServerCommunicationThread extends Thread {
    ...
    public ServerCommunicationThread(Socket socket, TextView serverHistoryTextView) {
        if (socket != null) {
            this.socket = socket;
            Log.d(Constants.TAG, "[SERVER] Created communication thread with: "
    ...
    public void run() {
        try {
            ...
            while (isRunning) {
                ...
                // TODO exercise 7a
                Log.d(Constants.TAG, "[SERVER] Waiting to receive data with prefix \""
                        + expectedWordPrefix + "\" on socket " + socket);
                String request = requestReader.readLine();
                final String finalizedRequest1 = request;
                serverHistoryTextView.post(new Runnable() {
                    @Override
                    public void run() {
                        serverHistoryTextView.setText("Server received word "
                                + finalizedRequest1 + " from client\n"
                ...
                if (Constants.END_GAME.equals(request)) {
                    ...
                    if ((Utilities.wordValidation(request)) && (request.length() > 2)
                            && (expectedWordPrefix.isEmpty() || request.startsWith(expectedWordPrefix))) {
                        String wordPrefix = request.substring(request.length() - 2);
                        List<String> wordList = Utilities.getWordListStartingWith(wordPrefix);

                        if (wordList.isEmpty()) {
                            Log.d(Constants.TAG, "[SERVER] Sent \""
                                    + Constants.END_GAME + "\" on socket " + socket);
                            responsePrintWriter.println(Constants.END_GAME);
                            serverHistoryTextView.post(new Runnable() {
                                @Override
                                public void run() {
                                    serverHistoryTextView.setText("Server sent word \""
                                            + Constants.END_GAME
                                            + "\" to client because it was locked out\n"
                                            + serverHistoryTextView.getText().toString());
                        ...
                        serverHistoryTextView.post(new Runnable() {
                            @Override
                            public void run() {
                                serverHistoryTextView.setText("Server sent back the word "
                                        + finalizedRequest2
                                        + " to client as it is not valid!\n"
                                        + serverHistoryTextView.getText().toString());
